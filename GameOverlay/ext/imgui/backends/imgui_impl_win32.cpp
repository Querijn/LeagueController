...
static void ImGui_ImplWin32_UpdateMouseData()
{
    ...
    if (is_app_focused)
    {
        ...
        if (io.WantSetMousePos)
        {
            ...
            if ((io.ConfigFlags & ImGuiConfigFlags_ViewportsEnable) == 0)
                Leacon_ProfilerEval(::ClientToScreen(focused_window, &pos));
            Leacon_ProfilerEval(::SetCursorPos(pos.x, pos.y));
        }
        ...
        if (!io.WantSetMousePos && !bd->MouseTracked && has_mouse_screen_pos)
        {
            ...
            if (!(io.ConfigFlags & ImGuiConfigFlags_ViewportsEnable))
                Leacon_ProfilerEval(::ScreenToClient(bd->hWnd, &mouse_pos));
            Leacon_ProfilerEval(io.AddMousePosEvent((float)mouse_pos.x, (float)mouse_pos.y));
        }
    }
    ...
    if (has_mouse_screen_pos)
        if (HWND hovered_hwnd = ::WindowFromPoint(mouse_screen_pos))
            if (ImGuiViewport* viewport = Leacon_ProfilerEvalRet(ImGui::FindViewportByPlatformHandle((void*)hovered_hwnd)))
                mouse_viewport_id = viewport->ID;
    Leacon_ProfilerEval(io.AddMouseViewportEvent(mouse_viewport_id));
}
...
void    ImGui_ImplWin32_NewFrame()
{
    ...
    RECT rect = { 0, 0, 0, 0 };
    Leacon_ProfilerEval(::GetClientRect(bd->hWnd, &rect));
    io.DisplaySize = ImVec2((float)(rect.right - rect.left), (float)(rect.bottom - rect.top));
    if (bd->WantUpdateMonitors)
        Leacon_ProfilerEval(ImGui_ImplWin32_UpdateMonitors());

    // Setup time step
    INT64 current_time = 0;
    Leacon_ProfilerEval(::QueryPerformanceCounter((LARGE_INTEGER*)&current_time));
    ...
    Leacon_ProfilerEval(ImGui_ImplWin32_UpdateGamepads());
}
