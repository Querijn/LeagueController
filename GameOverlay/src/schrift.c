...
#define FILE_MAGIC_TTCF           0x74746366
...
//static int
//init_font_collection(SFT_FontCollection* collection)
//{
//	uint_fast32_t scalerType, head, hhea;
//
//	if (!is_safe_offset(collection, 0, 12))
//		return -1;
//	/* Check for a compatible scalerType (magic number). */
//	scalerType = getu32(font, 0);
//	if (scalerType != FILE_MAGIC_ONE && scalerType != FILE_MAGIC_TWO)
...
/*static inline int
is_safe_offset(SFT_FontCollection* collection, uint_fast32_t offset)
{
	if (offset > font->size) return 0;
	if (font->size - offset < margin) return 0;
	return 1;
