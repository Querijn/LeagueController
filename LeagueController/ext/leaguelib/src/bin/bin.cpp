...
namespace LeagueLib
{
	...
	u32 FNV1Hash(const std::string& a_String);
...
#pragma optimize("", on)
	...
	static BinVariable ReadMat4(File::Handle file, size_t& offset)
	{
		...
		for (int x = 0; x < 4; x++)
		{
			for (int y = 0; y < 4; y++)
			{
				auto& t_Data = resultMatrix[x][y];
				file->Get(t_Data, offset);
			}
		}
	...
	BinVariable ConstructType(File::Handle& file, size_t& offset, Type type)
	{
		...
		{
			char buffer[2048];
			sprintf(buffer, "Unknown type %d in %s", (int)type, file->GetFullName().c_str());
